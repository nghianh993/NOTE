...
namespace S3Operations
{
    class CreateS3WebsiteTask
    {
        ...
        async Task EnableWebHosting(IAmazonS3 s3Client, string bucketName)
        {
            ...
            Console.WriteLine("Enabling web hosting on the bucket...");

            var putRequest = new PutBucketWebsiteRequest()
            {
                BucketName = bucketName,
            ...
            await s3Client.PutBucketWebsiteAsync(putRequest);

            var getRequest = new GetBucketWebsiteRequest()
            {
                BucketName = bucketName
            };
            await s3Client.GetBucketWebsiteAsync(getRequest);
        ...
        async Task AllowAccessFromWeb(IAmazonS3 s3Client, string bucketName)
        {
            ...
            Console.WriteLine("Adding a bucket policy to allow traffic from the internet...");
            await s3Client.PutBucketPolicyAsync(bucketName, bucketPolicy);
            Console.WriteLine("You can access the website at: ");
            await s3Client.GetBucketWebsiteAsync(bucketName);
            // End TODO 10
        }
